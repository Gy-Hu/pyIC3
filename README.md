...
Also requires: `yosys`, `yosys-abc`, `aigtoaig` for Verilog-to-AIGER conversion.

```bash
# From Verilog source (auto-converts via Yosys, generates LLM hints, runs IC3):
python run_llm.py -v design.v --top module_name

# From pre-converted AIGER + symbol map:
python run_llm.py --aag model.aag --map model.map -v design.v

# Compare vanilla vs LLM-guided side by side:
python run_llm.py --aag model.aag --map model.map -v design.v --compare

# Run all BlockSys benchmarks (requires blocksys-benchmark/converted_with_map/):
python run_llm.py --batch

# Vanilla only (no LLM):
python run_llm.py --aag model.aag --no-llm
```
